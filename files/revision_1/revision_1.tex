...
\frame{
  ...
  \framesubtitle{}
  \begin{itemize}
    \item Expressions like \texttt{2 + 3/5.0} are evaluated in a particular order, determined by {\color{red}operator precedence}.\vspace*{2mm}
    \item Division has a {\color{red}higher precedence} than addition, so \texttt{3/5.0} is evaluated {\color{red}first}, and \texttt{2} is then added on afterwards.\vspace*{2mm}
    \item If we wanted \texttt{2 + 3} to be evaluated first, then we need to use {\color{red}parentheses}: \texttt{(2 + 3)/5.0}.\vspace*{2mm}
